...
if ($action == 'addpayment' && !empty($facid)) {
    ...
    if ($error == 0) {
        ...
        if ($global_result > 0) {
            ...
            // ==============================================================================
            //                  AJOUT - CLASSEMENT DE LA FACTURE                      
            // ==============================================================================
            // On recharge la facture pour avoir le montant total payé à jour
            $facture->fetch($facid);
            
            // On vérifie si le restant à payer est nul (avec une petite tolérance)
            if (($facture->total_ttc - $facture->getSommePaiement()) < 0.01)
            {
                // On utilise la méthode standard de Dolibarr pour classer la facture comme "Payée"
                $result_setpaid = $facture->set_paid($user);
                if ($result_setpaid < 0) {
                    setEventMessage("Erreur lors du classement de la facture en 'Payée' : " . $facture->error, 'errors');
                    $global_result = -1; // Important pour annuler la transaction
                }
            }
            // ==============================================================================
            //                           FIN DE L'AJOUT                                 
        ...
        // On revérifie le résultat global au cas où le classement aurait échoué
        if ($global_result > 0) {
            $db->commit();
